...
public class WordAbbrivatino2 {
}

class IndexedWord_2 {
    public String word;
    public int index;
    public IndexedWord_2(final String word, int index) {
        this.word = word;
        this.index = index;
    }
}
...
class Solution_2 {
    public List<String> wordsAbbreviation(List<String> words) {
        ...
        Map<String, List<IndexedWord_2>> abbrevToIndexedWords = new HashMap<>();

        for (int i = 0; i < words.size(); ++i) {
            final String abbrev = getAbbrev(words.get(i), 0);
            abbrevToIndexedWords.putIfAbsent(abbrev, new ArrayList<>());
            abbrevToIndexedWords.get(abbrev).add(new IndexedWord_2(words.get(i), i));
        }

        for (List<IndexedWord_2> indexedWords : abbrevToIndexedWords.values()) {
