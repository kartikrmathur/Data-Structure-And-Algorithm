package CompanyWIseQuestion.Snapchat.WordAbbrivation;
...
class IndexedWord_1 {
    public String word;
    public int index;
    public IndexedWord_1(final String word, int index) {
        this.word = word;
        this.index = index;
    }
}
...
class Solution_1 {
    public List<String> wordsAbbreviation(List<String> words) {
        ...
        Map<String, List<IndexedWord_1>> abbrevToIndexedWords = new HashMap<>();

        for (int i = 0; i < words.size(); ++i) {
            final String abbrev = getAbbrev(words.get(i), 0);
            abbrevToIndexedWords.putIfAbsent(abbrev, new ArrayList<>());
            abbrevToIndexedWords.get(abbrev).add(new IndexedWord_1(words.get(i), i));
        }

        for (List<IndexedWord_1> indexedWords : abbrevToIndexedWords.values()) {
            TrieNode root = new TrieNode();
            for (IndexedWord_1 iw : indexedWords)
